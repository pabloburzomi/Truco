...
class Computadora(Juego):     
    ...
    def acepta_envido(self, envido_ia, eleccion_persona):        
        ...
        if eleccion_persona == 0:   #SI LA PERSONA CANTA ENVIDO
            if envido_ia >= 20 and envido_ia < 25:
                ...
                if retruco == 2: # Si hay azar
                    retruco = self.retruca_envido(eleccion_persona)
                    return retruco
                return 1000 # Es un Quiero sin retrucar
            elif envido_ia > 24: # Si el envido de la IA es mayor a 24 retruca
                retruco = self.retruca_envido(eleccion_persona)
                return retruco
            elif envido_ia < 20: # Si el envido de la IA es menor a 20 puede mentir o no querer
                if random.randrange(4) == 3 and envido_ia > 4:  # Hacemos mentir a la IA
                    if retruco == 2: # azar
                        retruco = self.retruca_envido(eleccion_persona)
                        return retruco
                else:
                    return None # Devuelve un "No Quiero"
            ...
            if envido_ia >= 25 and envido_ia < 29:
                ...
                if retruco == 2: # Si hay azar y si la persona elije Real Envido
                    retruco = self.retruca_envido(eleccion_persona)
                    return retruco
            ...
            if envido_ia >= 29:
                ...
                #print("Computadora: No quiero") # Borrar esta linea, esta duplicada en el MAIN
                return None
        elif eleccion_persona == 3: # Si la pesona Juega sin Cantar
            return 2000 # La Persona se arrepiente y no el 2do menú no quiere cantar
    ...
    def canta_envido(self,envido_ia):
        ...
        if envido_ia >= 20 and envido_ia < 25:
            ...
            if random.randrange(4) == 3 and envido_ia > 4:  # Hacemos mentir a la IA
                ...
                return None # Unica posiblididad que no quiera       
    ...
    def juega(self,en_juego,carta_persona, mano_persona,ganador_primera_mano):
        ...
        if carta_persona != None:
            valor_carta_persona = self.verificar_truco(carta_persona)
        else:
            canto_ia = en_juego
            return canto_ia, carta_ia
            
        if mano_persona != True or (mano_persona == True and ganador_primera_mano == None): #Si es la primera mano puede cantar aunque la persona no haya cantado
            if valor_carta_ia < valor_carta_persona:
                canto_ia = self.canta_truco(en_juego)
            elif valor_carta_ia > valor_carta_persona and ganador_primera_mano == "Computadora":
                canto_ia = self.canta_truco(en_juego)
            else:
                canto_ia = en_juego
        else:
    ...
    def juega_truco(self, carta_persona):
        ...
        if carta_persona != None:
            ...
            valor_carta_persona = None

        for carta in self.mano:
            valor_carta_ia = self.verificar_truco(carta)

            if valor_carta_ia < valor_mejor_carta_momentanea:
                valor_mejor_carta_momentanea = valor_carta_ia
                mejor_carta_definitiva = carta
        ...
        if valor_carta_persona == None: #Si le toca jugar a ia primero juega la mejor carta
            ...
            for carta in self.mano:
                ...
                if valor_carta_ia > peor_carta_momentanea:
                    peor_carta_momentanea = valor_carta_ia
                    peor_carta_definitiva = carta                
